...
func Record(w http.ResponseWriter, r *http.Request) {
	...
	//uuid := record.IPAddress+"-"+t.Add(time.Hour * 8).Format("20060102-150405")
	...
	if _, err := w.Write([]byte("id " + uuid)); err != nil {
...
func DrawPNG(w http.ResponseWriter, r *http.Request) {
	...
	drawer.Dot = freetype.Pt(x, canvasHeight/8)
	...
	x = canvasWidth/4 - p.Round()/2
	drawer.Dot = freetype.Pt(x, canvasHeight*9/20)
	...
	x = canvasWidth/4 - p.Round()/2
	drawer.Dot = freetype.Pt(x, canvasHeight*7/10)
	...
	x = canvasWidth/4 - p.Round()/2
	drawer.Dot = freetype.Pt(x, canvasHeight*24/40)
	...
	// TimeStamp
	drawer.Dot = freetype.Pt(6, canvasHeight-ctx.PointToFixed(14).Round()-60)
	drawer.DrawString("Result ID: " + uuid + "   Time: " + record.Timestamp.Add(time.Hour * 8).Format("2006-01-02 15:04:05"))
